...
- **Flagged the unverified REST `identify` response example**, which contradicts the CLI-verified shape in `references/platform/identify.md` (no `data` wrapper; `document_type` is an object, not a string).
