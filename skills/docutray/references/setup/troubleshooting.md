...
| `Non-interactive mode requires --api-key flag or api-key argument` | Bare `docutray login` invoked without a TTY (agent shell or CI) | Preferred: `docutray login --oauth` (CLI ≥ 0.3.1, drives the browser flow without exposing the key). Fallbacks: `docutray login --api-key dt_live_...`, the positional form `docutray login dt_live_...`, or set `DOCUTRAY_API_KEY` in the environment. |
