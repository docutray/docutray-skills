...
| `Non-interactive mode requires --api-key flag or api-key argument` | `docutray login` invoked without a TTY (agent shell or CI) | Use `docutray login --api-key dt_live_...`, the positional form `docutray login dt_live_...`, or set `DOCUTRAY_API_KEY` in the environment. |
...
- **`docutray status` shows `credential_source: env` but the key looks wrong.** The shell-exported `DOCUTRAY_API_KEY` is in effect; either correct the export or `unset` it to fall back to the config file.
